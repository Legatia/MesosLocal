 ...
 * Smart Contract:
   ...
   * Mints 40,000 PLNLOG (Logistics Vouchers) to the Client's Wallet.
 * Note: The Client now holds 40,000 PLNLOG. They cannot cash this out. They can only send it to a Whitelisted address.
## Step 3: Payment (The "Restricted Transfer")
The US Client clicks "Pay Invoice #PL-992".
 * Action: Client sends 40,000 PLNLOG to the Trucker’s Wallet.
 * Solana Transfer Hook (The Guard):
   * Check: Is the Receiver (Trucker) on the Whitelist? YES.
   * Result: Transaction Approved.
 * Time: < 1 Second.
 * Status: The Trucker sees "40,000 PLNLOG" in their app instantly. The Client gets a "Paid" receipt.
 ...
 * The Trucker spends PLNLOG at a Partner Merchant (also Whitelisted).
 ...
 * What Happens:
   1. Trucker burns 35,000 PLNLOG → triggers our backend.
   ...
   4. Partner handles USDC → PLN conversion and wires to trucker's bank.
 * Result: Trucker receives **34,825 PLN** (after 0.5% fee) in their bank account.
...
| Trucker | +Invoice (Unpaid) | +34,825 PLN (Cash) 
 +5,000 PLNLOG (Fuel) | Paid instantly. |
# 🔍 Why this is Legal (Compliance Check)
 * No "Money" Creation: You didn't print money. You issued a voucher backed by USDC.
 * No "Open Loop": The Client couldn't send the PLNLOG to a friend. The Transfer Hook blocked it.
 * No "Banking" for Client: The Client couldn't withdraw the PLNLOG to their bank. It was "Spend Only."
